...
image = cv2.imread(r"C:\Users\Dell\Downloads\Chistiano ronaldo.jpg")  #comment this for wikipedia results
p,q,r= np.shape(image)                              #comment this for wikipedia results
...
for i in range(0,len(unique_array)):
    hv_array[i] = round(((cdf_array[i] - cdf_min)*255)/((p*q) - cdf_min))
...
print("p x q x r  = %d x %d x %d"%(p,q,r))            #comment this for wikipedia results
